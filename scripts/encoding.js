...
fs.readFile('./airdropdata.csv', async (err, data) => {
  ...
  const airdropFactory = await hre.ethers.getContractFactory("AirdropPush");
  ...
  for (let i = 0; i < addresses.length; i += pageSize) {
    const batch = await airdropFactory.interface.encodeFunctionData(method, [FWB_PRO_ADDRESS, addresses.slice(i, i + pageSize), amounts.slice(i, i + pageSize)])
    console.log(`Batch ${i}`)
